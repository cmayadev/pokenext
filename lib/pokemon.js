...
export const getPokemonData = async (number) => {
    ...
    const entry = speciesData.flavor_text_entries.filter((flavor_text_entries) => {
        if( (flavor_text_entries.version.name === 'pearl')) return flavor_text_entries.flavor_text;
    })[0].flavor_text;
